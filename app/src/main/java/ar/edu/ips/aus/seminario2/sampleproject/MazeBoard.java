...
public class MazeBoard {
    ...
    // move single player
    public boolean movePlayer(BoardPiece.Direction dir) {
        boolean moved = false;
        switch(dir) {
            case NORTH:
                if (player.getY() > 0 &&
                        (getPiece(player.getX(), player.getY()).openTo(BoardPiece.Direction.NORTH) &
                         getPiece(player.getX(), player.getY()-1).openTo(BoardPiece.Direction.SOUTH))) {
            ...
            case SOUTH:
                if (player.getY() < getHeight()-1 &&
                        (getPiece(player.getX(), player.getY()).openTo(BoardPiece.Direction.SOUTH) &
                                getPiece(player.getX(), player.getY()+1).openTo(BoardPiece.Direction.NORTH))) {
                    player.setY(player.getY()+1);
                    moved = true;
                }
                break;
            case WEST:
                if (player.getX() > 0 &&
                        (getPiece(player.getX(), player.getY()).openTo(BoardPiece.Direction.WEST) &
                                getPiece(player.getX()-1, player.getY()).openTo(BoardPiece.Direction.EAST))) {
            ...
            case EAST:
                if (player.getX() < getWidth()-1 &&
                        (getPiece(player.getX(), player.getY()).openTo(BoardPiece.Direction.EAST) &
                                getPiece(player.getX()+1, player.getY()).openTo(BoardPiece.Direction.WEST))) {
                    player.setX(player.getX()+1);
                    moved = true;
                }
                break;
